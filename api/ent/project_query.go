...
func (pq *ProjectQuery) QueryClub() *ClubQuery {
	query := &ClubQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) QueryProjectClub() *ProjectClubQuery {
	query := &ProjectClubQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) QueryProjectLog() *ProjectLogQuery {
	query := &ProjectLogQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) WithClub(opts ...func(*ClubQuery)) *ProjectQuery {
	query := &ClubQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) WithProjectClub(opts ...func(*ProjectClubQuery)) *ProjectQuery {
	query := &ProjectClubQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withProjectClub = query
	return pq
}
...
func (pq *ProjectQuery) WithProjectLog(opts ...func(*ProjectLogQuery)) *ProjectQuery {
	query := &ProjectLogQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context) ([]*Project, error) {
	...
	if query := pq.withCompany; query != nil {
		...
		for i := range nodes {
			if nodes[i].company_project == nil {
				continue
			}
			fk := *nodes[i].company_project
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "company_project" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withClub; query != nil {
		...
		nodeids := make(map[int][]*Project)
		for i := range nodes {
			if nodes[i].club_project == nil {
				continue
			}
			fk := *nodes[i].club_project
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "club_project" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withProjectClub; query != nil {
		...
		query.Where(predicate.ProjectClub(func(s *sql.Selector) {
			s.Where(sql.InValues(project.ProjectClubColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := pq.withProjectLog; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Project)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.ProjectLog(func(s *sql.Selector) {
			s.Where(sql.InValues(project.ProjectLogColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.project_project_log
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "project_project_log" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
